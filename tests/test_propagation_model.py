...
class TestHFPropagationModel:
    ...
    @pytest.mark.xfail(
        reason="pre-existing failure carried from hf-timestd's 28-failure "
               "baseline (predates the split); tracked, not a port regression",
        strict=False)
...
class TestIonoDataService:
    ...
    def test_chapman_profile(self):
        from hamsci_dsp.ionosphere.data_service import IonoDataService, IonoGridPoint
