"""
Unit tests for hamsci_dsp.propagation.tec_validator
...
from hamsci_dsp.propagation.tec_validator import TECValidator
from hamsci_dsp.propagation.tec_geometry import calculate_midpoint, convert_slant_to_vertical
...
def validator():
    ...
    with patch('hamsci_dsp.ionosphere.model.IonosphericModel') as MockModel:
        mock_iono = MagicMock()
...
class TestInitialization:
    def test_default_ionex_dir(self):
        # Library contract (split §5.2 DI): no client-path default —
        # None flows to IonosphericModel as IONEX-disabled; the
        # consuming client supplies its own path.
        with patch('hamsci_dsp.ionosphere.model.IonosphericModel'):
            v = TECValidator()
        assert v.ionex_dir is None
