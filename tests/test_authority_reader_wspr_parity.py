#!/usr/bin/env python3
"""wspr-recorder's AuthorityReader tests, moved here verbatim when its private
216-line copy of the reader collapsed onto this module (2026-09-10).  They run
against the shared implementation so the collapse lost no coverage; the
sidecar block shape they assert is the one every sigmond client writes."""
...
from hamsci_dsp.timing import AuthorityReader, AuthoritySnapshot
...
class TestCanonicalTimingAuthority(unittest.TestCase):
    ...
    def test_standalone_timing_authority_block(self) -> None:
        from hamsci_dsp.timing import standalone_timing_authority
