...
class RaytraceEngine:
    ...
    def __init__(self, receiver_lat: "float | None" = None,
                 receiver_lon: "float | None" = None):
        # Coordinates are optional so the library contract the original
        # hamsci-dsp stub promised — construct cheaply, check
        # is_available() — survives the engine replacement (split §5.2).
        # Ray-tracing methods that need a receiver location call
        # _require_receiver() and raise a clear error when unset.
    ...
    def _require_receiver(self) -> None:
        if self.receiver_lat is None or self.receiver_lon is None:
            raise ValueError(
                "RaytraceEngine was constructed without a receiver "
                "location; pass receiver_lat/receiver_lon to use "
