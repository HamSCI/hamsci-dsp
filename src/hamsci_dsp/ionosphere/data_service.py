...
3. Caching data locally to minimize network requests
   - Cache directory: injected by the client (temp dir by default)
...
GIRO (primary real-time):
    - Station list: https://lgdc.uml.edu/common/DIDBFastStationList (HTML;
      ...
      hamsci_dsp/data/giro_stations.tsv when the page is down).
...
from hamsci_dsp.propagation.tec_geometry import great_circle_distance
...
# Cache settings
# DI (split §5.2): no client-path default in the library — None means
# a temp-dir cache; the consuming client supplies its own path.
        ...
        with cls._lock:
            if cls._instance is None:
                ...
                if ((cache_dir is not None
                     and str(inst.cache_dir) != str(Path(cache_dir))) or
