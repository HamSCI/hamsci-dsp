...
from hamsci_dsp.constants import C_KM_MS as C_LIGHT_KM_MS
from hamsci_dsp.ionosphere.solar import solar_position
from hamsci_dsp.geometry import height_from_path
...
class IonosphericModel:
    ...
    def __init__(
        ...
        Args:
            ...
            ionex_dir: Directory containing IONEX files (None disables
                IONEX lookup; the consuming client supplies its path)
        """
        ...
        # DI (split §5.2): no client-path default in the library — None
        # disables IONEX lookup; hf-timestd's shim supplies its path.
        self.ionex_dir = Path(ionex_dir) if ionex_dir else None
    ...
    def _find_ionex_file(self, timestamp: datetime) -> Optional[Path]:
        ...
        filename patterns.
        """
        if self.ionex_dir is None or not self.ionex_dir.exists():
            return None
...
def get_hmF2(
    ...
    Usage:
        from hamsci_dsp.ionosphere.model import get_hmF2
...
def calculate_ionospheric_delay(
    ...
    Usage:
        from hamsci_dsp.ionosphere.model import calculate_ionospheric_delay
...
# ---------------------------------------------------------------------------
# Lightweight profile API (pre-split hamsci-dsp surface, kept verbatim):
# IonoState / ionosphere_state — IRI when importable, parametric fallback.
# The full IonosphericModel above is the split-era engine (hf-timestd).
# ---------------------------------------------------------------------------
