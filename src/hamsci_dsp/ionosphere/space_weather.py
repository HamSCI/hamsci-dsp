"""
Space-Weather Service (hamsci_dsp.ionosphere) — moved from hf-timestd — near-real-time solar & geomagnetic indices.
...
class SpaceWeatherService:
    ...
    @classmethod
    def get_instance(cls, cache_dir: Optional[str] = None,
    ...
    def __init__(self, cache_dir: Optional[str] = None, session=None):
        if cache_dir is None:
            import tempfile
            cache_dir = str(Path(tempfile.gettempdir()) / "hamsci_dsp_iono_cache")
        ...
        # The HTTP session is INJECTED (split design: the network layer
        # stays with the caller; hamsci-dsp gains no requests dependency).
        # session=None means nothing is fetchable — parse/cache still work.
