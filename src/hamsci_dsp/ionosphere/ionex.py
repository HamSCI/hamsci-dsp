...
Canonical home of IONEXParser since the hf-timestd split (design §5.2);
moved verbatim from ``hf_timestd.core.ionex_parser`` (itself the P-H18
consolidation of the old ``scripts/ionex_integration.py`` exec-loading).
hf-timestd re-exports from here during the shim era.
            ...
            if 'START OF TEC MAP' in line:
                ...
                try:
                    ...
                    if len(parts) >= 1:
                        ...
                        # Longitude fill position for the data lines that
                        # follow.  BUG FIX (found by the split's new parse
                        # tests, 2026-08-24): this was a per-line local
                        # reset to 0, so of the 5 continuation lines per
                        # latitude (73 lons, 16 values/line) only the
                        # first 16 columns were ever filled — IONEX VTEC
                        # read ~0 over most of the globe.
                        lon_fill = 0
                except Exception as e:
                ...
                try:
                    ...
                    # Fill longitude values for this latitude, continuing
                    # from where the previous continuation line stopped.
                    for i, val in enumerate(tec_values):
                        if lon_fill + i < len(lon_grid):
                            current_map[lat_idx, lon_fill + i] = val
                    lon_fill += len(tec_values)
