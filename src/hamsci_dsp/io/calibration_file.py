...
#: Injected by the consuming client (split §5.2: the one true circular
#: dep — hf_timestd.version — becomes an injected string).
PRODUCT_VERSION: str = "unknown"


def set_product_version(version: str) -> None:
    """Install the consuming product's version string (shim-era DI)."""
    global PRODUCT_VERSION
    PRODUCT_VERSION = str(version)


def _get_version() -> str:
    """Return the consuming product's version string."""
